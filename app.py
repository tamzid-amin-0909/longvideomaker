...
# --- Perfect UI Styling (Your Requested UI + Refined Download) ---
st.markdown("""
    <style>
    ...
    /* SPECIFIC MODERN DOWNLOAD UI - Indigo Gradient and Pulse Effect */
...
with st.container():
    ...
    # The 'accept' parameter helps prompt for images on mobile/modern browsers
    ...
    if uploaded_file:
        ...
        st.markdown("""
            <i class="fas fa-image" style="font-size: 32px; margin-bottom: 12px; color: #4f46e5;"></i>
            <span style="font-size: 11px; font-weight: 900; text-transform: uppercase; letter-spacing: 0.1em;">Select Image</span>
            <span style="font-size: 9px; color: #94a3b8; margin-top: 5px;">TAP TO OPEN GALLERY</span>
        """, unsafe_allow_html=True)
    ...
    if st.button("Start Rendering"):
        if not uploaded_file:
            ...
            st.session_state.curr_name = custom_name

    # 4. Modern Download UI (Persistent until new upload)
    if 'ready' in st.session_state and os.path.exists(st.session_state.file):
        st.markdown("<div style='margin-top: 20px;'></div>", unsafe_allow_html=True) 
        with open(st.session_state.file, "rb") as f:
            st.download_button(
                label=f"📥 Save {st.session_state.curr_name.upper()}.MP4",
                data=f,
                file_name=f"{st.session_state.curr_name}.mp4",
                mime="video/mp4",
